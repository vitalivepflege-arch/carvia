        ...
        <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
          ...
          <div className="space-y-6">
            ...
            <Card title="Operations Log">
              <div className="mt-5 space-y-4">
                {workspace.operationLogs.length === 0 ? (
                  <div className="rounded-3xl bg-[var(--surface-muted)] p-5">
                    <p className="font-medium text-[var(--navy)]">No team operations logged yet</p>
                    <p className="mt-2 text-sm text-[var(--foreground-muted)]">
                      Role changes, reassignments, rebalance batches, and capacity edits will appear here once the team starts operating the control center.
                    </p>
                  </div>
                ) : (
                  workspace.operationLogs.map((entry) => (
                    <div key={entry.id} className="rounded-3xl border border-[var(--border)] bg-[var(--surface-muted)] p-4">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <p className="font-medium text-[var(--navy)]">{entry.summary}</p>
                          <p className="mt-1 text-sm text-[var(--foreground-muted)]">
                            {entry.actorUserName ?? "Unknown operator"} | {entry.actionType.replaceAll("_", " ")}
                          </p>
                        </div>
                        <StatusPill tone="info">{entry.targetType}</StatusPill>
                      </div>
                      <p className="mt-3 text-xs uppercase tracking-[0.16em] text-[var(--foreground-muted)]">
