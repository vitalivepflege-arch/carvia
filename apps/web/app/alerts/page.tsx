      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            { label: "Capacity", value: String(alertCenter.summary.capacityCount), delta: "Queues at risk of overload" }
          ].map((item) => (
        ...
        <Card title="Capacity Risks">
          <div className="mt-5 space-y-4">
            {alertCenter.capacityAlerts.length === 0 ? (
              <div className="rounded-3xl bg-[var(--surface-muted)] p-5">
                <p className="font-medium text-[var(--navy)]">No capacity risks right now</p>
                <p className="mt-2 text-sm text-[var(--foreground-muted)]">
                  Current Buyer, Sales, and Admin queues are within configured WIP and SLA boundaries.
                </p>
              </div>
            ) : (
              alertCenter.capacityAlerts.map((alert) => (
                <div key={alert.id} className="rounded-3xl border border-[var(--border)] bg-[var(--surface-muted)] p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <StatusPill tone={severityTone[alert.severity as keyof typeof severityTone]}>{alert.value}</StatusPill>
                  </div>
                  {alert.recommendedRole ? (
                    <p className="mt-3 text-sm text-[var(--foreground)]">
                      Recommended rebalance target: {alert.recommendedRole}
                    </p>
                  ) : null}
                </div>
              ))
