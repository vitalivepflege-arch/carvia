...
export const closingStatusLabels = {
  CANCELLED: "Cancelled",
  COMPLETED: "Completed",
  NONE: "No closing",
...
export async function getClosingWorkspace(companyId: string) {
  const items = await prisma.watchlist.findMany({
    where: { companyId },
    orderBy: [{ closingUpdatedAt: "desc" }, { offerUpdatedAt: "desc" }, { createdAt: "desc" }]
  });
